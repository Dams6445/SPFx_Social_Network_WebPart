...
export const InstaSpDiscovery = (props: IInstaSpDiscoveryProps) => {
    ...
    React.useEffect(() => {
        ...
        const getAccountData = async () => {
            const [_data] = await Promise.all([instaService.getSpDiscoveryData(accountName)]);
            // console.log("val reçue dans le TSX : ", _test)
            console.log("On y croit toujours !!! : ", _data)
            setDiscovery(_data)
        }
        getAccountData();
    }, [accountName]);

    if (discovery?.username != accountName)
    {
        return (
            <div className={styles.post}>
                <h1>Compte Instagram Inconnu</h1>
            </div>
        )
    }
    if (!discovery) {
